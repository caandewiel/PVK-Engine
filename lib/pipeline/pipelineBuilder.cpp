...
//            this->pipelineCreateInfo.setPDynamicState(&this->dynamicState);
            ...
            this->colorBlendAttachmentState.colorWriteMask = vk::ColorComponentFlagBits::eR | vk::ColorComponentFlagBits::eG | vk::ColorComponentFlagBits::eB | vk::ColorComponentFlagBits::eA;
        ...
        void Builder::loadShader(std::string filePath, vk::ShaderStageFlagBits stage, const char* entryPoint) {
            auto code = pvk::util::readFile(filePath);
            
            try {
                auto shaderModule = pvk::Context::getLogicalDevice().createShaderModule({
                ...
                shaderStage.setFlags(vk::PipelineShaderStageCreateFlags());
                shaderStage.setStage(stage);
                shaderStage.setModule(shaderModule);
                shaderStage.setPName(entryPoint);
        ...
        void Builder::clearShaderModules() {
            for (auto &shaderStage : this->shaderStages) {
                pvk::Context::getLogicalDevice().destroyShaderModule(shaderStage.module, nullptr);
            }
