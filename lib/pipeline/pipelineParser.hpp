...
namespace pvk {
    ...
    json parseDefinition(const std::string &filePath) {
    ...
    std::vector<std::unique_ptr<DescriptorSet>> parseDescriptorSets(const json &jsonContent) {
        if (jsonContent.find(FIELD_DESCRIPTOR_SETS) == jsonContent.end()) {
            std::ostringstream exceptionMessage;
            exceptionMessage << "Index " << FIELD_DESCRIPTOR_SETS << " not found in pipeline definition.";
            ...
            for (auto &binding : descriptorSet[FIELD_BINDINGS]) {
                auto _binding =
                        std::make_unique<DescriptorBinding>(binding[FIELD_NAME].get<std::string>(),
                                                            ...
                                                            descriptorTypeMapping.at(
                                                                    binding[FIELD_TYPE].get<std::string>()),
                                                            shaderStageMapping.at(
                                                                    binding[FIELD_STAGE].get<std::string>()));

                _descriptorSet->bindings.emplace_back(std::move(_binding));
            }

            result.emplace_back(std::move(_descriptorSet));
        }
    ...
    std::unique_ptr<pvk::Pipeline> createPipelineFromDefinition(const std::string &filePath,
        ...
        auto jsonContent = parseDefinition(filePath);
        auto _descriptorSets = parseDescriptorSets(jsonContent);
        ...
        auto pipeline =
                std::make_unique<Pipeline>(pipelineBuilder.create(Context::getPipelineCache()),
