...
auto GLTFLoader::loadObject(vk::Queue &graphicsQueue, const std::string &filePath) -> std::unique_ptr<gltf::Object>
{
    ...
    //        buffer::create<Vertex,
    //        vk::BufferUsageFlagBits::eVertexBuffer>(graphicsQueue,
    //                                                                       object->vertexBuffer,
...
auto GLTFLoader::loadNode(const std::shared_ptr<tinygltf::Model> &model,
                          ...
                          vk::Queue &graphicsQueue,
                          gltf::Object &object,
                          std::shared_ptr<gltf::Node> parent) -> std::shared_ptr<gltf::Node>
{
    ...
    {
        resultNode->children.emplace_back(loadNode(model, primitiveLookup, child, graphicsQueue, object, resultNode));
    }
...
template <typename T> constexpr uint8_t getComponentType()
{
                               ...
                               gltf::Object &object)
{
    ...
    for (auto i = 0; i < static_cast<int>(model->materials.size()); i++)
    {
        ...
        object.materials[i] = std::move(_material);
    }
}
...
std::map<uint32_t, std::shared_ptr<gltf::Node>> GLTFLoader::initializeNodeLookupTable(
    std::vector<std::shared_ptr<gltf::Node>> &nodes)
{
...
inline std::vector<gltf::Primitive *> getAllPrimitive(const std::shared_ptr<gltf::Node> &node)
{
...
std::map<uint32_t, std::vector<gltf::Primitive *>> GLTFLoader::initializePrimitiveLookupTable(
    std::vector<std::shared_ptr<gltf::Node>> &nodes)
{
...
template <typename T> auto flatten(std::vector<std::future<std::vector<T>>> &&t) -> std::future<std::vector<T>>
{
...
auto GLTFLoader::loadPrimitives(const std::shared_ptr<tinygltf::Model> &model,
                                vk::Queue &graphicsQueue,
                                gltf::Object &object) -> std::vector<std::vector<std::unique_ptr<gltf::Primitive>>>
{
    ...
    for (auto &mesh : model->meshes)
    {
        ...
        for (auto &primitive : mesh.primitives)
        {
            ...
            const auto vertexCount = model->accessors[primitive.attributes.find(FIELD_VERTEX_POSITION)->second].count;
            ...
            auto _primitive =
                std::make_unique<gltf::Primitive>(currentVertexOffset, currentIndexOffset, vertexCount, indexCount);
